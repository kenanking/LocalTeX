...
use image::{imageops, ColorType, GrayImage, ImageEncoder, Luma, Rgba, RgbaImage};
...
const INTAKE_PAPER_SIZE: u32 = 82;
const INTAKE_PAPER_SCALE: u32 = 3;

#[derive(Clone, Copy, Debug, PartialEq)]
pub struct IntakePaperSpec {
    pub degrees: f32,
    pub edge_seed: u64,
}

pub struct IntakePaperVariants {
    pub neutral: RgbaImage,
    pub working: RgbaImage,
}

pub fn intake_paper_spec(batch_gen: u64, item_key: u64, slot: usize) -> IntakePaperSpec {
    let seed = mix64(
        batch_gen
...
fn intake_paper_base(img: &RgbaImage, seed: u64) -> (RgbaImage, Vec<(f32, f32)>) {
    ...
    let thumb = thumbnail(img, inner, inner);
    imageops::overlay(
        &mut paper,
        &thumb,
